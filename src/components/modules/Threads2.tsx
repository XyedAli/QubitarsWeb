...
float lineFn(vec2 st, float width, float perc, float offset, vec2 mouse, float time, float amplitude, float distance, float phase) {
    ...
    float time_scaled = time / 10.0 + phase + (mouse.y - 0.5) * 1.0;
    float blur = smoothstep(split_point, split_point + 0.05, st.y) * perc;

    float ynoise = mix(
        Perlin2D(vec2(st.y + perc, time_scaled) * 2.5),
        Perlin2D(vec2(st.y + time_scaled, time_scaled) * 3.5) / 1.5,
        st.y * 0.3
    );
...
const Threads2 = ({
  ...
  }, [color, amplitude, distance, phase, enableMouseInteraction]);
