...
const OurClient: React.FC<FunMomentsProps> = ({
    testimonials,
    heading,
    description
}) => {
            ...
            <div className="our-client-slider">
                <Slider ref={sliderRef} {...settings}>
                    {testimonials.map((item, index) => (
                        <div
                            key={item.id}
                            className={`relative transition-all duration-300 mt-[47px] ${currentSlide === index ? " scale-95 xl:scale-86 z-10" : "top-10 scale-85 blur-xs"
                                }`}>
                            <Image
                                src={item.image3}
                                alt={item.author}
                            ...
                            <div className="bg-white border border-[#E5E5E5] rounded-tl-3xl rounded-br-3xl shadow-lg flex flex-col justify-between h-[230px] md:h-[250px] lg:h-[340px] xl:h-[350px] max-w-7xl w-full pt-6 lg:pt-8 xl:pt-12">
                                ...
                                <div className="flex flex-col w-full flex-1">
                                    ...
                                    <div className="px-4 lg:px-6 xl:px-8 flex-1 flex items-start lg:items-center">
                                        <p
                                            className={combine("font-normal text-zinc-800 italic transition-all duration-300 w-full", styles.p3)}
                                        >
                                                ...
                                                {!readMoreStates[item.id] ? (
                                                    <>
                                                        ...
                                                        {item.desp.split(' ').length > 25 && (
                                                            <span className="text-zink underline text-[12px] font-medium hover:text-[#D6AB61] cursor-pointer ml-1" onClick={(e) => toggleReadMore(item.id, e)}>
                                                                Read More
                                                            </span>
                                                        )}
                                                    </>
                                                    ...
                                                    <>
                                                        {item.desp}
                                                        <span className="text-zink underline text-[12px] font-medium hover:text-[#D6AB61] cursor-pointer ml-1 block" onClick={(e) => toggleReadMore(item.id, e)}>
                                                            Read Less
                                                        </span>
                                                    </>
                                                )}
                                            ...
                                            <span className="hidden md:block lg:hidden">
                                                {!readMoreStates[item.id] ? (
                                                    <span className="testimonial-text-clamp" onClick={(e) => toggleReadMore(item.id, e)}>
                                                        {item.desp} <span className="text-zink underline text-[12px] font-medium hover:text-[#D6AB61] cursor-pointer">Read More</span>
                                                    </span>
                                                ) : (
                                                    <>
                                                        {item.desp}
                                                        <span className="text-zink underline text-[12px] font-medium hover:text-[#D6AB61] cursor-pointer ml-1 block mt-1" onClick={(e) => toggleReadMore(item.id, e)}>
                                                            Read Less
                                                        </span>
                                                    </>
                                                )}
                                ...
                                <div className="flex items-end justify-between px-4 xl:px-6 lg:px-8 pb-4 md:pb-5 lg:pb-6">
                                    ...
                                    {item.logo && (
                                        <img src={item.logo} alt="Company Logo" className="h-6 md:h-8 lg:h-10 w-auto" />
                                    )}
                                </div>
                            </div>
                        </div>
