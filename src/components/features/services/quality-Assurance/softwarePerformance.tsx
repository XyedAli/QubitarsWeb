...
function HeaderGraphic() {
  return (
    <div className="relative shrink-0 w-[140px] h-[100px] sm:w-[160px] sm:h-[112px] hidden md:block" aria-hidden>
  ...
  return (
    ...
    >
      <div className="w-full">
        ...
        />
        <div className={combine("grid grid-cols-1 md:grid-cols-2 gap-x-10 lg:gap-x-16 xl:gap-x-12 overflow-hidden")}>
          {SOFTWARE_PERFORMANCE_ITEMS.map((item, index) => {
            ...
            return (
              <article
                key={id}
                className={combine(
                  "py-5 xl:px-10 lg:py-7 xl:py-11 border-t-2 border-gray-300",
                  index < 2 && "",
                  index % 2 === 0 && ""
                )}
              >
                <div className={combine(styles.flexBetween, styles.flexitems, "gap-4 mb-3 lg:mb-5")}>
        ...
        <div
          className={combine(
            styles.flexitems,
            "flex-col sm:flex-row sm:items-start sm:justify-between gap-8 lg:gap-12 pt-4 md:pt-6 xl:pt-11 border-t-2 border-gray-3 00"
          )}
        >
          <div className="min-w-0 flex-1 max-w-2xl">
            <h2
              id="software-performance-heading"
              className={combine(styles.h2, "font-bold text-[#1a1a1a] mb-3 lg:mb-5 tracking-tight")}
            >
              Usability Testing
            </h2>
