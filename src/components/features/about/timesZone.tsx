...
const MAP_ICON_CLASS = "w-6 h-8 sm:w-7 sm:h-9 drop-shadow-lg map-icon";
const MAP_PING_CLASS = "w-6 h-8 sm:w-7 sm:h-9 opacity-40 map-icon-ping";
...
function generateResponsiveCSS(): string {
  ...
  const mobileOverrides = locations
    .filter((loc) => loc.topMobile || loc.leftMobile)
    .map((loc) => `.loc-${loc.id} { --top: ${loc.topMobile ?? loc.top} !important; --left: ${loc.leftMobile ?? loc.left} !important; }`);
  cssRules.push(`@media (max-width: 639px) {
...
function PinIcon({ color, className }: { color: string; className: string }) {
  return (
    <svg width="24" height="32" viewBox={PIN_VIEWBOX} fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
...
const TimesZone = () => {
  ...
  const responsiveCSS = useMemo(() => generateResponsiveCSS(), []);
            ...
            {locations.map((loc, index) => (
              ...
              >
                <div className="relative">
                  <PinIcon color={loc.color} className={MAP_ICON_CLASS} />
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 animate-ping pointer-events-none">
                    <PinIcon color={loc.color} className={MAP_PING_CLASS} />
                  </div>
                </div>
