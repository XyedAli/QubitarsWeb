...
const BenefitCard = ({ icon, value, label, index, isActive, onMouseEnter, onMouseLeave, onClick }: BenefitCardProps) => {
  ...
  const staggerOffset = isMiddleColumn && !isBottomRow ? "mt-0 md:mt-6 lg:mt-12" :
    isMiddleColumn && isBottomRow ? "mt-0 md:mt-3 lg:mt-4" : "";

  const negativeMargin = isBottomRow && !isMiddleColumn ? "-mt-0 md:-mt-3 lg:-mt-8" :
    isBottomRow && isMiddleColumn ? "-mt-0 md:-mt-4 lg:-mt-5" : "";
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col p-4 xl:p-6 ">
        {/* Icon Box */}
        <div className={`w-16 h-16 md:w-17 md:h-17 xl:w-20 xl:h-20 rounded-full bg-white border-2 transition-all duration-300 ${styles.flexCenter} mb-3 md:mb-4 lg:mb-4 ${isActive ? "border-[#F05C22]" : "border-transparent group-hover:border-[#F05C22]"
          }`}>
          {isSvg ? (
            <img
              src={icon}
        ...
        {/* Title */}
        <h3 className={`text-[19px] md:text-[24px] lg:text-[20px] xl:text-[25px] text-blue font-bold mb-2 font-outfit leading-tight`}>
          {value}
        </h3>

        {/* Description */}
        <p className={`text-[12px] md:text-[14px] lg:text-[13px] xl:text-[14px] leading-relaxed font-inter ${isActive ? "text-gray-700" : "text-gray-600"
          }`}>
          {label}
        </p>
      </div>
