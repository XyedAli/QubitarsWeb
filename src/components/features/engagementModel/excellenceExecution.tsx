...
const ExcellenceExecution = () => {
    ...
    const cards = executionExcellenceSection.cards.map((card) => ({
        ...card,
        image: (engagementImages as Record<string, string>)[card.imageKey],
    }));
    ...
    const sliderSettings = {
        ...
        autoplaySpeed: 4000,
        pauseOnHover: true,
        responsive: fiveToFourToThreeToTwoToOne,
    };
                    ...
                    {cards.map((card) => (
                        <div key={card.number} className="h-full">
                            <div className="relative rounded-xl overflow-hidden group min-h-[320px] md:min-h-[340px] lg:min-h-[380px] xl:min-h-[430px]">
                                ...
                                />
                                <div
                                    className="absolute inset-0 bg-gradient-to-b from-black/60 via-black/20 to-transparent transition-opacity duration-300 group-hover:opacity-0"
                                    aria-hidden
                                />
                                ...
                                <div className="absolute inset-0 flex flex-col justify-start p-3 md:p-4 lg:p-5 xl:p-6">
                                    ...
                                    <p
                                        className={`${styles.p2} mt-2 text-white/95 font-inter leading-relaxed opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-[1200ms] ease-out `}
                                    >
                                        {card.description}
                                    </p>
                                </div>
